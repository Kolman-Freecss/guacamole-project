...
CREATE TABLE guacamole_user_connection_comment
(
    ...
    CONSTRAINT `guacamole_user_connection_comment_ibfk_1`
        FOREIGN KEY (`user_id`)
            REFERENCES `guacamole_user` (`user_id`)
            ON DELETE CASCADE,

    CONSTRAINT `guacamole_connection_comment_ibfk_1`
...
--
-- Table of user connection comment permissions. Each user connection comment permission grants a user or user group specific access to a connection comment.
--
...
FROM (SELECT 'guacadmin' AS username, 'guacadmin' AS affected_username, 'READ' AS permission
      ...
      UNION
      SELECT 'guacadmin' AS username, 'guacadmin' AS affected_username, 'ADMINISTER' AS permission) permissions
         JOIN guacamole_entity ON permissions.username = guacamole_entity.name AND guacamole_entity.type = 'USER'
